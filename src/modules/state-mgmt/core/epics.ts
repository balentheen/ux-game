...
export const coreGetEpicCheckForUpdates: Epic<IAction, IAction, IRootState, IEpicDependencies> = (action$, state$, deps) =>
  action$.pipe(
    ...
    mergeMap(() => interval(ENV.CHECK_FOR_UPDATES_INTERVAL).pipe(
...
export const coreGetEpicBootstrap: Epic<IAction, IAction, IRootState, IEpicDependencies> = (action$, state$, deps) =>
  action$.pipe(
    ...
    mergeMap(() => of(
      userState.actions.setListStart([state$.value.auth.currentUserId]),
      todoState.actions.setListStart({ page: 1, limit: ENV.PAGINATION.LIMIT })
    ).pipe(
